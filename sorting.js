...
console.log('pi string array converted to nums array: ', stringsToNums(strNums))

// With the same numbers, find the sum of the even values
// evens = strNums.filter(num => num % 2 === 0)
// console.log(evens)
...
let getState = (state) => state.weather_state_name
const weatherStates = weather.map(getState)
...
const idealTemp = (arr) => {
    let getId = (id) => id.id
    const tempFilter = arr.filter(wea => wea.min_temp === 15.915)
    return tempFilter.map(getId).toString()
}
